...
import { showBackDropStore, hideBackDropStore,openModalShared, closeModalShared, showAlert, hideModalPedido } from "../globalStore/globalStore.js";
import { showStore, listStore, resetFormularioStore, handleFormStore, setSiguienteNumeroOrden } from "./proveedoresOrdenesStore.js";

import { URL } from "../../constants/constantGlogal.js";
const namespace_api      = "/api/suppliers/"; 
const seguiente_numero   = "ordenes/siguiente-numero/";
const ordenes_create     = "ordenes/create/";
const ordenes_list       = "ordenes/";
const ordenes_by_proveedor = "ordenes/by-proveedor/";
...
export const getSiguienteNumeroOrdenThunk = () => {
  return async (dispatch, getState) => {
    ...
    const options = {
      method: "GET",
      url: `${URL}${namespace_api}${seguiente_numero}`, //Endpoint a crear en backend
      headers: {
    ...
    try {
      ...
      if (response.status === 200) {
        const siguienteNumero = response.data.siguiente_numero; // Ej: "OP-00001"
        
        dispatch(setSiguienteNumeroOrden({ numero: siguienteNumero }));
      }
    } catch (error) {
      console.error("❌ Error al obtener siguiente número de orden:", error);
      
      // Si falla, generar uno temporal
      const numeroTemporal = `OP-${String(Date.now()).slice(-5)}`;
      dispatch(setSiguienteNumeroOrden({ numero: numeroTemporal }));
    }
  };
};

export const createOrdenThunk = (ordenData) => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'POST',
            url: `${URL}${namespace_api}${ordenes_create}`, // Ajusta según tu endpoint
            headers: {
                Authorization: `Bearer ${token}`,
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                // Recargar lista de órdenes si tienes un getAllOrdenesThunk
                await dispatch(getAllOrdenesThunk());
                
                await dispatch(hideModalPedido());
            ...
            await dispatch(
                showAlert({
                    type: "error",
                    title: "Error al crear la orden",
                    text: error.response?.data?.error || "Ocurrió un error al crear la orden.",
                })
            );
...
export const getAllOrdenesThunk = ({page = 1,  pageSize = 10, search = "", start_date = "", end_date = "",} = {}) => {
  ...
  return async (dispatch, getState) => {
    ...
    const options = {
      method: "GET",
      url: `${URL}${namespace_api}${ordenes_by_proveedor}?${params.toString()}`,
      headers: {
        Authorization: `Bearer ${token}`,
      },
    };
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        await dispatch(listStore({ ordenes: ordenes.results }));
      } else {
            ...
            if(response.status == 200){
                await dispatch(showStore(
                                            { idProveedor           : response.data.id ?? '',
                                              ...
                                              contacto_principal    : response.data.contacto_principal ?? '',
