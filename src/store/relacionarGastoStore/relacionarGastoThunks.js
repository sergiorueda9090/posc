...
import { showStore, listStore, resetFormularioStore, handleFormStore } from "./relacionarGastoStore.js";

import { URL } from "../../constants/constantGlogal.js";
const namespace_api      = "/api/gastos/";
const endpoint           = "list/record/";
const endpoint_delete    = "/record/delete/";
const endpoint_create    = "create/record/";
const endpoint_update    = "record/update/";
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (response.status === 200) {
        const relacionGastos = response.data;
        await dispatch(listStore({ relacionGastos }));
      } else {
...
export const showThunk= (id = "") => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'GET',
            url: `${ URL}${namespace_api}${id}/record/`,
            headers: {
              Authorization: `Bearer ${token}`
            }
          };
          ...
          try {
            ...
            if(response.status == 200){
                ...
                await dispatch(showStore(
                                            { idRelacionGasto       : response.data.id ?? '',
                                              gasto_id              : response.data.gasto_id ?? '',
                                              total_gasto           : response.data.total_gasto ?? '',
