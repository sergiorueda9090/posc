...
import { showStore,listStore,handleFormStore, resetFormularioStore } from "./reporteStore.js";

import { URL } from "../../constants/constantGlogal.js";
const namespace_api      = "/api/ventas/reporte/";
const endpoint           = "list/";
...
export const getAllThunks = ({page = 1,  pageSize = 10, search = "", start_date = "", end_date = "",} = {}) => {
  ...
  return async (dispatch, getState) => {
    ...
    const options = {
      method: "GET",
      url: `${URL}${namespace_api}?${params.toString()}`,
      headers: {
    ...
    try {
      ...
      if (response.status === 200) {
        const resumen_general = response.data;
        await dispatch(listStore({ resumen_general }));
      } else {
