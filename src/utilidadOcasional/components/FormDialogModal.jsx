...
import { createThunks, updateThunks } from "../../store/utilidadOcasionalStore/utilidadOcasionalThunks";
...
export const FormDialogModal = () => {
  ...
  const utilidadOcasionalStore = useSelector(state => state.utilidadOcasionalStore);
  ...
  const selectedOptionTarjetas = optionsTarjetas.find((opt) => opt.id == utilidadOcasionalStore.tarjeta_id) || null;
  ...
  const {
    ...
    setErrors,
    resetForm,
  } = useForm(utilidadOcasionalStore);
  ...
  const handleSubmit = (event) => {
    ...
    const isEdit = Boolean(formValues.idUtilidadOcasional);
    ...
    if (isEdit) {
      dispatch(updateThunks({ id: formValues.idUtilidadOcasional, ...data }));
    } else {
  ...
  return (
    <Dialog open={openModalStore} keepMounted onClose={handleClose} fullWidth maxWidth="md">
      <DialogTitle sx={{ padding: "16px 4px 0px 16px" }}>
         {formValues.idUtilidadOcasional ? "Editar Ajuste de saldo" : "Crear Ajuste de saldo"}
      </DialogTitle>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <DialogActions>
          ...
          <Button type="submit" variant="outlined" color="primary">
            { formValues.idUtilidadOcasional ? "Actualizar" : "Crear" }
          </Button>
        </DialogActions>
      </form>
    </Dialog>
