...
import { handleFormStoreThunk } from "../../store/ordenPedidoStore/ordenPedidoThunks";

export const SelectComponentProveedor = () => {
  const dispatch = useDispatch();
  
  const { proveedores } = useSelector((state) => state.proveedoresStore);
  const { proveedor_id } = useSelector((state) => state.ordenPedidoStore);
  ...
  useEffect(() => {
    if (proveedores?.results) {
      const newOptions = proveedores.results.map((prov) => ({
        value: prov.id,
        label: `${prov.nombre_proveedor} - ${prov.ciudad}`,
        data: prov,
      }));
